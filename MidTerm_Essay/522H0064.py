...
def task1():
    ...
    contours, hierarchy = cv.findContours(image=dillation, mode=cv.RETR_TREE, 
                                method=cv.CHAIN_APPROX_NONE)  

    for cnt in contours:
         cv.drawContours(image=frame_gray, contours=[cnt], contourIdx=0, color=255, thickness=cv.FILLED)
...
def task2():
    ...
    th1 = cv.adaptiveThreshold(img_gray, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 11, 2)
    ...
    def edit(img,iteration, kernels,lower):
        ...
        contours1, hierarchy1 = cv.findContours(dillation, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE)
